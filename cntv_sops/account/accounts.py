...
DEFAULT_CACHE_TIME_FOR_USER_UPDATE = settings.DEFAULT_CACHE_TIME_FOR_USER_UPDATE
...
class Account(AccountSingleton):
    ...
    def build_callback_url(self, request, jumpUrl):
        ...
        compare_scheme = (not login_scheme or login_scheme == current_scheme)
        compare_netloc = (not login_netloc or login_netloc == current_netloc)
        if compare_scheme and compare_netloc:
    ...
    def check_failed(self, request):
        ...
        if code == 'func_check':
            ...
            res_page = '/403.html'
        return render(request, res_page)
