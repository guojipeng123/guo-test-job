...
class PipelineParser(object):
    ...
    def __init__(self, pipeline_tree, circle_free=False, strict_converge=True):
    ...
    def get_act(self, act_id, subprocess_stack=None, root_pipeline_data=None):
        ...
        acts = filter(lambda x: x.id == act_id, subprocess.spec.activities)
        if not acts:
            raise exceptions.NodeNotExistException(u"act[act_id=%s] not found" % act_id)
        return acts[0]
