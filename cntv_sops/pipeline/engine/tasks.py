...
def start(process_id):
    ...
    if not action_result.result:
        logger.info('can not start pipeline(%s), message: %s' % (pipeline_id, action_result.message))
        return
...
def node_timeout_check(node_id, version, root_pipeline_id):
    ...
    if not state or state != states.RUNNING:
        logger.info('node %s %s timeout kill failed, node not exist or not in running' % (node_id, version))
        return
