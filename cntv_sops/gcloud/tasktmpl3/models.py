...
class TaskTemplateManager(BaseTemplateManager):
    ...
    def extend_classified_count(self, group_by, filters=None, page=None, limit=None):
        ...
        if group_by == AE.state:
            ...
            id_list = [x[0] for x in template_list.values_list("id")[(page - 1) * limit:page * limit]]
            ...
            for taskflow in taskflow_list:
                taskflow_dict[taskflow["template_id"]] = taskflow["instance_total"]
            relationship_dict = {}
            ...
            # 按起始时间、业务（可选）、类型（可选）查询各流程模板原子节点个数、子流程节点个数、网关节点数
            total = tasktmpl.count()
            groups = []
    ...
    def get_collect_template(self, biz_cc_id, username):
        template_list = self.filter(is_deleted=False, business__cc_id=biz_cc_id)
        user_model = get_user_model()
        collected_templates = user_model.objects.get(username=username).tasktemplate_set.all() \
            .values_list('id', flat=True)
        collected_templates_list = []
        for template in template_list:
            if template.id in collected_templates:
